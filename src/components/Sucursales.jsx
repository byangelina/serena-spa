...
const SANTIAGO = { lat: -33.45, lng: -70.67 }
const RADIO_MAX_KM = 100

// Fórmula de Haversine — distancia en línea recta entre dos puntos geográficos
function distanciaLinealKm(a, b) {
  const R = 6371
...
function Sucursales() {
  ...
  const [estadoRutas, setEstadoRutas] = useState("esperando") // esperando | cargando | listo | error | fuera_de_rango
  ...
  useEffect(() => {
    ...
    // Si el usuario está a más de 100 km de Santiago, no tiene sentido calcular rutas
    if (distanciaLinealKm(ubicacion, SANTIAGO) > RADIO_MAX_KM) {
      setEstadoRutas("fuera_de_rango")
      return
    }
  ...
  return (
    <section id="sucursales" style={{ background: "var(--blanco)", padding: "6rem 6%" }}>
      ...
      {/* Estado: usuario fuera del área de cobertura */}
      {estadoRutas === "fuera_de_rango" && (
        <p style={{ color: "var(--gris)" }}>
          Nuestras sucursales se encuentran en Santiago de Chile. Si planeas visitarnos,{" "}
          <a href="#contacto" style={{ color: "var(--verde-osc)" }}>contáctanos</a> y te ayudamos a coordinar tu visita.
        </p>
      )}
